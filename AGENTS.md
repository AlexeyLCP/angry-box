...
10. **AWG amnezia obfuscation — four P0 fixes (handshake-breaking), all verified by a real awg-quick tunnel on the test VPS (`TestE2E_Heavy_PerClientRouting`, PASS, `latest handshake: 5 seconds ago`).** Userspace (`System: false`) amnezia WORKS in our patched build — `patches/wireguard-go-awg-overlap.patch` fixes the upstream `chacha20poly1305` panic that crashed userspace amnezia; do NOT switch to `System: true` to "make amnezia work". The fixes:
    ...
    **Fixed audit gaps:** S3/S4/ITime now emitted (`AWGPreset` gained the fields, presets carry s3=22/s4=12/itime=130, `BuildAmneziaSection` copies them, client .conf writes `Itime`); H1-H4 now proper quadrant ranges (`GenAWGParams` wired into `GenerateAWGObfsMaterial`, persisted on `model.Chain.AWGH1..H4`, server↔client identical, width >= 1000); `RenderAWGHop` MTU 1280→1420 (matches all other AWG endpoints); **standalone AWG is multi-peer** (`NodeInbound.Tag` stable identity + `usersByInboundMap` thread users into `buildStandaloneInOut` → `buildAWGUserInboundMulti`, one peer per assigned user; `scheduleAutoApplyForUser` now also redeploys standalone nodes); **AWG takeover preserves peers+amnezia** (dedicated `renderAWGTakeoverConfig` builds the userspace endpoint directly from `ImportAWGConfigsViaClient`'s `AwgServerConfig`+`AwgPeerEntry` — server keypair/listen port/amnezia (JC/S1-S4/H1-H4/I1-I5) copied 1:1, all peers kept; bypasses `generateAWGUser` which pulled amnezia from a preset and hardcoded one peer; cutover now disables `awg-quick@awg0` to free the port, rollback re-enables it).
    **Still-open audit gaps:** `I1Packet` parsed but unused; **takeover'd AWG has no per-client routing** (the dedicated renderer does NOT create `model.User` entries for imported peers — VPN functionality is preserved, but `source_ip_cidr` per-client steering is unavailable on a takeover'd inbound until peers are materialized as users, a follow-up); **server-IP collision** if a node hosts BOTH a chain AWG entry (`10.8.0.1/32`) and a standalone AWG inbound (`10.8.0.1/32`) — needs per-inbound server IP allocation (follow-up). User `AWGAddress` reuse across inbounds on different nodes is safe.
