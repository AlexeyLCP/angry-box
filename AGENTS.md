...
**REVISION 2026-07-31 (AGENTS #5 — AWG protocol version selector + kernel-AWG3 horizon):** (1) Введён канонический селектор версии протокола AWG — `InboundProfile.AWGVersion` / `NodeInbound.AWGVersion` (`"1.5"` / `"2"` / `"3"`, см. `internal/domain/model/awg_version.go`). Таксономия: **1.x/1.5** = legacy (Jc/Jmin/Jmax/S1-S2/фикс.H1-H4, без CPS, degenerate H1-H4 "N-N"); **2.0** = текущий kernel+CPS дефолт (+S3-S4/I1-I5/H1-H4 ranges/Itime); **3.0** = +HeaderProtectionKey/ContentPaddingAddition/RekeyAfterTime. `AWG3Mode bool` остаётся legacy-aliasом (`EffectiveAWGVersion()` → `"3"` когда `AWG3Mode==true || AWGVersion=="3"`); миграция store НЕ нужна — reconciliation на лету. UI: dropdown версии на inbound (1.5 legacy / 2.0 kernel+CPS default / 3.0 header protection) заменил старый checkbox `awg3_mode` (`web/inbounds.go`, `inbounds.templ`). Все runtime-проверки рендера переключены с `ib.AWG3Mode` на `ib.EffectiveAWGVersion()==model.AWGVersion3` (`awg_deploy.go`, `awg_tun_overlay.go`, `merged_config.go`, `inbound_source.go`, `awg_inbound_material.go`), так что `AWGVersion="3"` (новый путь) и legacy `AWG3Mode=true` оба рендерят userspace endpoint. (2) **Вчера (2026-07-30) вышел kernel module с нативной поддержкой AWG3** — PR [#192 «feat: AmneziaWG 3.0»](https://github.com/amnezia-vpn/amneziawg-linux-kernel-module/pull/192) слит в `master` репо `amneziawg-linux-kernel-module` (kernel `wg_device` struct несёт `header_protection`/`content_padding_addition`/`rekey_after_time/timeout` `u16_range_t`; fix валидации Sx≥12 — коммиты `7304fbf`/`ff0aa32`, 2026-07-31). Это ЛОМАЕТ прежний абсолютный constraint «kernel rejects HPK» — **kernel-render path для AWG3 становится ВОЗМОЖЕН (slice 2, пока не сделан):** deps bump (`deps/amneziawg-src.tar.gz` от 30 июня устарел) + `RenderServerAWGConf`/`RenderExitServerAWGConf` emit HPK/CPM/RAT в `[Interface]` (как CPS, через `writeAmneziaConfLines` с новым флагом `includeAWG3`) + `awgTUNOverlayNeeded` вернуть true для kernel-AWG3 + `AWGTeardownInterfaces` реверс (kernel-AWG3 требует awg0 UP, не teardown) + capability detection (`awg version` на VPS) + fallback на userspace + E2E `TestE2E_Heavy_Protocol_AWG3_Kernel` на n1. **Срез 1 (этот коммит) оставляет AWG3 userspace** — инфраструктурная работа вынесена отдельно, т.к. требует Linux/WSL-сборки deps и живого E2E. (3) **Пресеты по версиям**: `AWGPreset.Version` (`"1.5"`/`"2"`/`"3"`); новые `*_awg3` пресеты (`maximum_stealth_2026_awg3`, `russia/iran/china_2026_awg3`) — HPK-on, S1-S4=24 (≥12 nonce-size), **H1-H4 минимизированы (12/12/12/12) как избыточные под HPK** (HPK fast-encrypt'ит low-entropy header fields, частично перекрывая роль H-markers — механизм другой, полностью не убираются, но fingerprint-критичность H-markers падает); CPS (I1-I5) и Jc ортогональны HPK, остаются. `GroupPresets` → optgroup `AWG · 3.0 (header protection)` ПЕРВЫМ (макс stealth), затем Robust/Stealth/Reality/XHTTP. `PresetSupportsAWGVersion` + `resolveAWGPresetForVersion` enforce preset↔version contract (v3 inbound не может silently отрендерить v2-пресет с S1-S4<12 → fallback на per-version default `defaultPresetForAWGVersion`). Тесты: `awg_version_test.go` (EffectiveAWGVersion reconciliation, resolver fallback/keep, material generation, profile propagation, S1-S4≥12). См. PROGRESS §43.
