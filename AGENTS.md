...
13. **Traffic status (live VPS, re-confirmed 2026-07-04):** Inter-node forwarding (XHTTP/Reality transport) **works** — `TestE2E_Heavy_Protocol_AWG_Kernel_2Hop` PASS (entry `tun-in` catch-all → `ch-...-out-www` chain outbound → exit XHTTP transport-in healthy). Kernel AWG handshake + balancer deploy **work** — `TestE2E_Heavy_Protocol_AWG_Kernel` PASS (awg0.conf pushed, `awg-quick@awg0` active, 0 userspace WG endpoints, systemd `After=awg-quick@awg0.service`). `TestE2E_Heavy_PerClientRouting` PASS — AWG handshake OK (`latest handshake: 5 seconds ago`), balancer architecture (Table=off, MASQUERADE, 0 userspace WG). Full client→internet egress through AWG tunnel was **not verified on a single VPS** (routing conflict: test client awge2e + server awg0 on same host — test artifact, not a product bug; production client is on a separate device). Per-client `source_ip_cidr` under TUN-overlay still needs real-VPS verify. See `docs/PROGRESS.md` §8.
