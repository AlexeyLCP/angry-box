...
10. **AWG amnezia obfuscation — four P0 fixes (handshake-breaking), all verified by a real awg-quick tunnel on the test VPS (`TestE2E_Heavy_PerClientRouting`, PASS, `latest handshake: 5 seconds ago`).** Userspace (`System: false`) amnezia WORKS in our patched build — `patches/wireguard-go-awg-overlap.patch` fixes the upstream `chacha20poly1305` panic that crashed userspace amnezia; do NOT switch to `System: true` to "make amnezia work". The fixes:
    ...
    **Still-open audit gaps:** `I1Packet` parsed but unused; **takeover'd AWG has no per-client routing** (the takeover does NOT create `model.User` entries for imported peers — VPN functionality is preserved, but `source_ip_cidr` per-client steering is unavailable on a takeover'd inbound until peers are materialized as users, a follow-up); **server-IP collision** if a node hosts BOTH a chain AWG entry (`10.8.0.1/32`) and a standalone AWG inbound (`10.8.0.1/32`) — needs per-inbound server IP allocation (follow-up). User `AWGAddress` reuse across inbounds on different nodes is safe.
...
13. **Traffic status (live VPS, re-confirmed + egress VERIFIED 2026-07-04):** Inter-node forwarding (XHTTP/Reality transport) **works** — `TestE2E_Heavy_Protocol_AWG_Kernel_2Hop` PASS. Kernel AWG handshake + balancer deploy **work** — `TestE2E_Heavy_Protocol_AWG_Kernel` PASS. **Full client→internet egress VERIFIED end-to-end** — server-2 (kernel AWG client, 10.8.0.99) → entry (server-1) → exit (server-3) → internet; `curl --interface awg0 ifconfig.me → 23.251.133.38` (exit's public IP, Brussels BE). 3 routing bugs found & fixed in this session (see `docs/PROGRESS.md` §11): (1) exit `MASQUERADENetwork` now covers BOTH user `10.8.0.0/24` AND balancer-link `10.10.0.0/24` subnets (balancer-routed egress was un-NAT'd → internet couldn't route responses back); (2) `tunIncludeInterfaces` now includes `awg-exit-nX` (without it sing-box `bind_interface: awg-exit-nX` dials timed out — the SYN-ACK arrived on the kernel awg-exit-nX but sing-box never captured it); (3) `RenderServerAWGConf`/`RenderExitAWGConf`/`RenderExitServerAWGConf` PostUp now emit `sysctl net.ipv4.conf.<iface>.rp_filter=0` (awg-quick recreates the interface on every restart, resetting rp_filter to 1, which dropped return traffic). Per-client `source_ip_cidr` under TUN-overlay still needs real-VPS verify. See `docs/PROGRESS.md` §8/§11.
