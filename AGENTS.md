...
11. **Hysteria2 transport is FROZEN — do NOT implement or fix.** Like TUIC (#6), Hysteria2 inter-node transport is unresearched. The `TransportHysteria2` constant + UI dropdown exist, but `buildChainRoleInOut` (`merged_config.go`) has NO Hysteria2 case — it now refuses loudly (a `MergeReport.Warning` + no inbound/outbound emitted) instead of silently falling back to Reality. Do NOT write a Hysteria2 builder, do NOT add Hysteria2 E2E tests, do NOT attempt Hysteria2-transport bug fixes. AWG is the main product protocol — keep the focus there. Revisit Hysteria2 only after an explicit user request. Also note (kernel-AWG rework, 2026-07-03): user-facing AWG servers (chain entry, standalone, exit) now use kernel `awg-quick@awg0` + sing-box TUN-overlay — NOT userspace `WireGuardEndpoint` (that path is unstable under amnezia; see `docs/PROGRESS.md` §1.A). The legacy CLI `Backend.ApplyConfig` standalone-AWG path (`cmd/angry-box/main.go:673` via `RenderAWGHop`) STILL uses a userspace endpoint — a known follow-up to convert to `pushConfigWithAWG` or deprecate in favor of `ApplyMergedNode`. Do NOT revert the kernel-AWG rework.
