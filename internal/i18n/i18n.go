...
var locales = map[string]map[string]string{
	"en": {
		...
		"Nothing to take over": "Nothing to take over",
		"From:": "From:",
		...
		"applied": "applied",
		"pending": "pending",
		"Pending means Apply, not Check": "Pending = click Apply. Check only pings the node.",
		"never": "never",
		...
		"invalid offsite path": "invalid offsite path",
		...
		"Port": "Port",
		"Leave empty to auto-assign a free port": "Leave empty to auto-assign a free port",
		"Profile:": "Profile:",
		"Protocols": "Protocols",
		"QR": "QR",
		...
		"Use global default (Jc=120, premium)": "Use global default (Jc=120, premium)",
		"Preset hint": "Robust (Jc≤10) = reliable handshake on budget VPS; Stealth (Jc=120) = max anti-DPI on premium channels. AWG 3.0 mode adds header protection on top of any preset.",
		"User": "User",
		...
		"unique, not yet used": "unique, not yet used",
		"Clone to new VPS": "Clone to new VPS",
		"New node ID is required": "New node ID is required",
		"Clone this node's config to a new VPS with fresh identity (new keys/UUID). Serves the same users + same exit topology.": "Clone this node's config to a new VPS with fresh identity (new keys/UUID). Serves the same users + same exit topology.",
		// P2a — offsite backup.
		"Offsite backup": "Offsite backup",
		"Push an encrypted copy of the panel to an offsite SSH target on a schedule. The blob is passphrase-encrypted — restorable without this host's master key (which never leaves this host).": "Push an encrypted copy of the panel to an offsite SSH target on a schedule. The blob is passphrase-encrypted — restorable without this host's master key (which never leaves this host).",
		"Backup target host": "Backup target host",
		...
		"0 = 6h default": "0 = 6h default",
		"Offsite backup enabled": "Offsite backup enabled",
		"Backup now": "Backup now",
		"only for encrypted (ABBKP1) offsite blobs": "only for encrypted (ABBKP1) offsite blobs",
		"backup: offsite target not configured": "backup: offsite target not configured",
		"Backup sent to offsite target": "Backup sent to offsite target",
		...
		// v0.8 — first-class inbounds.
		"Add inbound": "Add inbound",
		"Edit inbound": "Edit inbound",
		"Dest / SNI": "Dest / SNI",
		"Reality dest hint": "Handshake target (e.g. www.microsoft.com). Empty = from the obfuscation preset / panel default.",
		"Public key": "Public key",
		"Short ID": "Short ID",
		"Reality keys hint": "Generated per node on deploy. The client URI uses this public key.",
		"Reality keys after deploy": "Public key and Short ID appear here after the inbound is deployed to a node.",
		"TLS protocol needs domain": "NaiveProxy and TrustTunnel need a TLS domain on the node (Utilities → set domain → Install all → Issue cert). Creating without a domain is refused.",
		"no TLS domain": "no TLS domain",
		"FakeTLS domain": "FakeTLS domain",
		"FakeTLS hint": "Set Dest / SNI above (e.g. disk.yandex.ru). Empty = disk.yandex.ru.",
		"Preset": "Preset",
		"not deployed": "not deployed",
		"(%d users)": "(%d users)",
		"Delete this inbound profile? It will be removed from all nodes.": "Delete this inbound profile? It will be removed from all nodes.",
		"Unbind this inbound from its chain first, then delete.": "Unbind this inbound from its chain first, then delete.",
		"No inbounds yet. Create one and deploy it onto your nodes — then reference it from a chain.": "No inbounds yet. Create one and deploy it onto your nodes — then reference it from a chain.",
		"Obfuscation preset": "Obfuscation preset",
		"Default preset": "Default preset",
		"Deploy on nodes": "Deploy on nodes",
		"No nodes yet — add nodes first.": "No nodes yet — add nodes first.",
		"Checking a node deploys the inbound there with fresh per-node credentials; unchecking removes it (refused while a chain references it).": "Checking a node deploys the inbound there with fresh per-node credentials; unchecking removes it (refused while a chain references it).",
		"Protocol is locked while the inbound is deployed (credentials are protocol-specific)": "Protocol is locked while the inbound is deployed (credentials are protocol-specific)",
		...
		"No manual route rules yet": "No manual route rules yet",
		"Comment": "Comment",
		"On": "On",
		"Off": "Off",
		"Enable/disable": "Enable/disable",
		"Delete this route rule?": "Delete this route rule?",
		"Add rule": "Add rule",
		"Edit rule": "Edit rule",
		"Save rule": "Save rule",
		"Cancel edit": "Cancel edit",
		"Geo source hint": "GeoSite/GeoIP categories come from SagerNet sing-geosite / sing-geoip (same sources as 3x-ui). The panel downloads .srs files and pushes them to the node — the node never fetches GitHub.",
		"Match type": "Match type",
		"Domain suffix": "Domain suffix",
		"Full domain": "Full domain",
		"Domain keyword": "Domain keyword",
		"IP CIDR": "IP CIDR",
		"Direct (bypass proxy)": "Direct (bypass proxy)",
		"Block (reject)": "Block (reject)",
		"Via outbound tag": "Via outbound tag",
		"Match values": "Match values",
		"Outbound tag": "Outbound tag",
		"Apply to users": "Apply to users",
		"All users": "All users",
		"Leave empty for all users. AWG users are matched by their tunnel IP (works through chains); other protocols by auth identity (entry node only).": "Leave empty for all users. AWG users are matched by their tunnel IP (works through chains); other protocols by auth identity (entry node only).",
		"Effective route table": "Effective route table",
		"Effective route table hint": "The generated sing-box route section for this node (manual + system rules, first match wins).",
		"No route rules in the generated config": "No route rules in the generated config",
		...
		"TLS domain hint": "The node's primary domain (A-record must point at the node). Protocol subdomains, the subscription and the panel relay derive from it.",
		"Set a TLS domain to enable the utility bundle.": "Set a TLS domain to enable the utility bundle.",
		"Set a TLS domain before installing utilities": "Set a TLS domain before installing utilities",
		"Utility": "Utility",
		"Install": "Install",
		"Uninstall": "Uninstall",
		"Install all": "Install all",
		"Issue/renew certificate": "Issue/renew certificate",
		"Sync subscriptions": "Sync subscriptions",
		"Install the subscription statics utility first": "Install the subscription statics utility first",
		"Subscription files re-rendered and pushed": "Subscription files re-rendered and pushed",
		"Install caddy + acme and set a TLS domain first": "Install caddy + acme and set a TLS domain first",
		"Uninstall this utility?": "Uninstall this utility?",
		"stale": "stale",
		"stale hint": "Pushed artifacts lag the store — re-apply",
		"SANs: ": "SANs: ",
		"SSH connect failed: ": "SSH connect failed: ",
		"Invalid TLS domain — use a bare hostname like node1.example.com": "Invalid TLS domain — use a bare hostname like node1.example.com",
		"Caddy (SNI router, 80/443)": "Caddy (SNI router, 80/443)",
		"ACME (acme.sh certificates)": "ACME (acme.sh certificates)",
		"Fake site (camouflage)": "Fake site (camouflage)",
		"Subscription statics": "Subscription statics",
		"Owns ports 80/443: routes TLS by SNI to the protocol inbounds, passes everything else to Reality, serves the site and subscriptions.": "Owns ports 80/443: routes TLS by SNI to the protocol inbounds, passes everything else to Reality, serves the site and subscriptions.",
		...
		"Bot token": "Bot token",
		"Bot admin IDs": "Admin Telegram IDs (comma-separated)",
		"Bot notify chat": "Notification chat ID",
		"Bot restart note": "The bot restarts when settings are saved.",
		"Bot help": "Commands: /status — my status, /config — subscription links. Admin: /nodes, /users, /online, /link <user>.",
		"Bot start hint": "Send /start <code> to bind. The code is issued by the operator via /link. Your Telegram ID: %d",
		"Bot bound": "Bound to user %s. /status shows your status, /config your subscriptions.",
		"Bot code not found": "Unknown or expired code. Ask the operator for a fresh one (/link).",
		"Bot not bound": "This account is not bound to a user. Send /start <code> with a code from the operator.",
		"Bot config no nodes": "No node serves subscriptions yet (install the sub utility). Your token:",
		"Bot config links": "Your subscriptions:",
		"Bot link usage": "Usage: /link <user name or ID>",
		"Bot user not found": "User %q not found",
		"Bot link issued": "Code for %s: %s — the user should send: /start %s",
		"Bot online none": "No users online (%d active)",
		"Bot online list": "Online %d/%d: %s",
		"Bot nodes none": "No nodes registered",
		"Bot users summary": "Users: %d (active %d, on hold %d, expired %d)",
		"Bot expiry warning": "User %s expires %s (in ~%dh)",
		...
		"0 = no expiry": "0 = no expiry",
		"Data limit (GB)": "Data limit (GB)",
		"0 = unlimited": "0 = unlimited",
		"No chains yet": "No chains yet",
		"%d users created (prefix %q) — refresh to see them": "%d users created (prefix %q) — refresh to see them",
		"Count must be 1..100": "Count must be 1..100",
		"no users were created (check chains/store)": "no users were created (check chains/store)",
		"Panel relay: ON": "Panel relay: ON",
		...
		"Panel import reads the panel's SQLite DB and brings over its users, inbounds and routing rules, then stops the panel service. The raw DB is backed up first. Wipe stops the panel without importing.": "Panel import reads the panel's SQLite DB and brings over its users, inbounds and routing rules, then stops the panel service. The raw DB is backed up first. Wipe stops the panel without importing.",
		"Import this panel's data? The panel service will be stopped.": "Import this panel's data? The panel service will be stopped.",
		"Import panel": "Import panel",
		"Stop the panel without importing?": "Stop the panel without importing?",
		"Wipe panel": "Wipe panel",
		"Panel import done": "Panel import done",
		"Imported: %d inbound(s), %d user(s), %d routing rule(s).": "Imported: %d inbound(s), %d user(s), %d routing rule(s).",
		"Panel DB download failed: ": "Panel DB download failed: ",
		"Panel DB parse failed: ": "Panel DB parse failed: ",
		"Panel DB backed up failed (continuing): ": "Panel DB backup failed (continuing): ",
		"Panel import needs the built-in SSH client (download support).": "Panel import needs the built-in SSH client (download support).",
		"save node: ": "save node: ",
		"Import details": "Import details",
		"Next: review the imported inbounds and Apply the node to deploy sing-box.": "Next: review the imported inbounds and Apply the node to deploy sing-box.",
		"Panel import result": "Panel import result",
		"Panel wipe": "Panel wipe",
		"Subscription": "Subscription",
		"Sub page hint": "Paste a vpn:// line into AmneziaVPN, or add ?format=clash / ?format=base64 for apps.",
		"Amnezia vpn://": "Amnezia vpn://",
		"Share links": "Share links",
		"AWG outbound": "AWG outbound",
		"AWG outbound hint": "Paste a client .conf or vpn:// to run this node as a client of another AWG server (awgo-N, Table=off).",
		"Add AWG outbound": "Add AWG outbound",
		"invalid port": "invalid port",
		"no free port available": "no free port available",
		"inbound profile not found": "inbound profile not found",
		"Profile saved, but removal on %s was refused: a chain references it there": "Profile saved, but removal on %s was refused: a chain references it there",
		"Protocol change requires recreating the profile (delete + create): deployed credentials are protocol-specific": "Protocol change requires recreating the profile (delete + create): deployed credentials are protocol-specific",
		"Profile is in use by a chain on %s — edit the chain first": "Profile is in use by a chain on %s — edit the chain first",
		"After connect": "After connect",
		"Deploy sing-box-extended (binary)": "Deploy sing-box-extended (binary)",
		"Install AmneziaWG kernel module": "Install AmneziaWG kernel module",
		"Detect existing VPN (takeover wizard)": "Detect existing VPN (takeover wizard)",
		"binary/AWG install queued": "binary/AWG install queued",
		"Detect VPN": "Detect VPN",
		// v0.8 — chain levels editor.
		"Transport (inter-node)": "Transport (inter-node)",
		"AWG / AmneziaWG (single-node levels only)": "AWG / AmneziaWG (single-node levels only)",
		"Transport preset (inter-node obfuscation)": "Transport preset (inter-node obfuscation)",
		"Levels (entry first, exit last)": "Levels (entry first, exit last)",
		"+ Add level": "+ Add level",
		"Traffic flows level by level. A level with several nodes balances across them per its strategy. The entry level references inbounds already deployed on its nodes — create them on the Inbounds page first.": "Traffic flows level by level. A level with several nodes balances across them per its strategy. The entry level references inbounds already deployed on its nodes — create them on the Inbounds page first.",
		"Level %d — Entry": "Level %d — Entry",
		"Level %d": "Level %d",
		"Round-robin (fallback)": "Round-robin (fallback)",
		"— choose inbound —": "— choose inbound —",
		"No inbound deployed on this node.": "No inbound deployed on this node.",
		"Create / deploy one": "Create / deploy one",
		"Chain name is required": "Chain name is required",
		"chain already exists": "chain already exists",
		"level %d has no nodes selected": "level %d has no nodes selected",
		"level %d: invalid strategy %q": "level %d: invalid strategy %q",
		"entry node %q has no inbound selected — deploy one on the Inbounds page first": "entry node %q has no inbound selected — deploy one on the Inbounds page first",
		"entry node %q: inbound profile %q not found": "entry node %q: inbound profile %q not found",
		"inbound %q is not deployed on node %q — deploy it on the Inbounds page first": "inbound %q is not deployed on node %q — deploy it on the Inbounds page first",
		"entry nodes must use inbounds of one protocol (%s vs %s)": "entry nodes must use inbounds of one protocol (%s vs %s)",
		"at least one level is required": "at least one level is required",
		"entry level is required": "entry level is required",
		"Topology": "Topology",
		"levels": "levels",
		...
		"AWG 3.1 (random trailers)": "AWG 3.1 (random trailers)",
	...
	"ru": {
		...
		"Use global default (Jc=120, premium)": "Глобальный по умолчанию (Jc=120, premium)",
		"Preset hint": "Robust (Jc≤10) = надёжный handshake на бюджетных VPS; Stealth (Jc=120) = максимальный anti-DPI на premium-каналах. Режим AWG 3.0 добавляет защиту заголовков поверх любого пресета.",
		...
		"Protocol": "Протокол",
		"Port": "Порт",
		"Leave empty to auto-assign a free port": "Пустое = свободный порт автоматически",
		...
		"Nothing to take over": "Нечего перехватывать",
		"From:": "От:",
		...
		"applied": "применён",
		"pending": "ожидает",
		"Pending means Apply, not Check": "«Ожидает» = нажми «Применить». «Проверить» только пингует ноду.",
		"never": "никогда",
		...
		"invalid offsite path": "недопустимый путь offsite-бэкапа",
		...
		// P1b — clone node.
		"Clone node": "Клонирование ноды",
		"Clone": "Клонировать",
		"New node ID": "ID новой ноды",
		"unique, not yet used": "уникальный, ещё не занят",
		"Clone to new VPS": "Клонировать на новый VPS",
		"New node ID is required": "ID новой ноды обязательно",
		"Clone this node's config to a new VPS with fresh identity (new keys/UUID). Serves the same users + same exit topology.": "Клонировать конфиг этой ноды на новый VPS со свежей идентичностью (новые ключи/UUID). Обслуживает тех же пользователей + ту же топологию выходов.",
		// P2a — offsite backup.
		"Offsite backup": "Внешний бэкап",
		"Push an encrypted copy of the panel to an offsite SSH target on a schedule. The blob is passphrase-encrypted — restorable without this host's master key (which never leaves this host).": "Отправлять зашифрованную копию панели на внешний SSH-таргет по расписанию. Блоб зашифрован паролем — восстанавливается без мастер-ключа этого хоста (который никогда его не покидает).",
		"Backup target host": "Хост внешнего таргета",
		"Remote path": "Удалённый путь",
		"Backup passphrase": "Пароль бэкапа",
		"leave blank to keep current": "пусто = текущий",
		"Backup interval (minutes)": "Интервал бэкапа (минуты)",
		"0 = 6h default": "0 = 6ч по умолчанию",
		"Offsite backup enabled": "Внешний бэкап включён",
		"Backup now": "Бэкап сейчас",
		"only for encrypted (ABBKP1) offsite blobs": "только для зашифрованных (ABBKP1) внешних блобов",
		"backup: offsite target not configured": "бэкап: внешний таргет не настроен",
		"Backup sent to offsite target": "Бэкап отправлен на внешний таргет",
		...
		// v0.8 — first-class inbounds.
		"Add inbound": "Добавить инбаунд",
		"Edit inbound": "Редактировать инбаунд",
		"Dest / SNI": "Dest / SNI",
		"Reality dest hint": "Цель handshake (например www.microsoft.com). Пусто = из пресета обфускации / дефолт панели.",
		"Public key": "Публичный ключ",
		"Short ID": "Short ID",
		"Reality keys hint": "Генерируется на ноде при деплое. Клиентская ссылка берёт этот ключ.",
		"Reality keys after deploy": "Публичный ключ и Short ID появятся здесь после деплоя инбаунда на ноду.",
		"TLS protocol needs domain": "NaiveProxy и TrustTunnel требуют TLS-домен на ноде (Утилиты → домен → Установить всё → Выпустить сертификат). Без домена создать нельзя.",
		"no TLS domain": "нет TLS-домена",
		"FakeTLS domain": "Домен FakeTLS",
		"FakeTLS hint": "Задайте Dest / SNI выше (например disk.yandex.ru). Пусто = disk.yandex.ru.",
		"Preset": "Пресет",
		"not deployed": "не развёрнут",
		"(%d users)": "(%d польз.)",
		"Delete this inbound profile? It will be removed from all nodes.": "Удалить этот инбаунд? Он будет снят со всех нод.",
		"Unbind this inbound from its chain first, then delete.": "Сначала отвяжи инбаунд от цепочки, потом удаляй.",
		"No inbounds yet. Create one and deploy it onto your nodes — then reference it from a chain.": "Инбаундов пока нет. Создайте инбаунд и разложите его по нодам — затем сошлитесь на него из цепочки.",
		"Obfuscation preset": "Пресет обфускации",
		"Default preset": "Пресет по умолчанию",
		"Deploy on nodes": "Развернуть на нодах",
		"No nodes yet — add nodes first.": "Нод пока нет — сначала добавьте ноды.",
		"Checking a node deploys the inbound there with fresh per-node credentials; unchecking removes it (refused while a chain references it).": "Галочка разворачивает инбаунд на ноде со свежими per-node кредами; снятие удаляет его (запрещено, пока цепочка ссылается на него).",
		"Protocol is locked while the inbound is deployed (credentials are protocol-specific)": "Протокол заблокирован, пока инбаунд развёрнут (креды специфичны для протокола)",
		...
		"No manual route rules yet": "Ручных правил пока нет",
		"Comment": "Комментарий",
		"On": "Вкл",
		"Off": "Выкл",
		"Enable/disable": "Включить/выключить",
		"Delete this route rule?": "Удалить это правило маршрутизации?",
		"Add rule": "Добавить правило",
		"Edit rule": "Изменить правило",
		"Save rule": "Сохранить правило",
		"Cancel edit": "Отменить правку",
		"Geo source hint": "Категории GeoSite/GeoIP — из SagerNet sing-geosite / sing-geoip (как в 3x-ui). Панель скачивает .srs и пушит на ноду — нода сама на GitHub не ходит.",
		"Match type": "Тип совпадения",
		"Domain suffix": "Суффикс домена",
		"Full domain": "Полный домен",
		"Domain keyword": "Ключевое слово домена",
		"IP CIDR": "IP CIDR",
		"Direct (bypass proxy)": "Напрямую (мимо прокси)",
		"Block (reject)": "Блокировать (reject)",
		"Via outbound tag": "Через outbound-тег",
		"Match values": "Значения",
		"Outbound tag": "Outbound-тег",
		"Apply to users": "Применить к пользователям",
		"All users": "Все пользователи",
		"Leave empty for all users. AWG users are matched by their tunnel IP (works through chains); other protocols by auth identity (entry node only).": "Оставьте пустым для всех пользователей. AWG-пользователи определяются по туннельному IP (работает сквозь цепочки); остальные протоколы — по auth-идентификатору (только на входной ноде).",
		"Effective route table": "Итоговая таблица маршрутов",
		"Effective route table hint": "Сгенерированная секция route для этой ноды (ручные + системные правила, первое совпадение побеждает).",
		...
		"TLS domain": "TLS-домен",
		"TLS domain hint": "Основной домен ноды (A-запись должна указывать на ноду). Из него выводятся поддомены протоколов, подписка и реле панели.",
		"Set a TLS domain to enable the utility bundle.": "Задайте TLS-домен, чтобы включить набор утилит.",
		"Set a TLS domain before installing utilities": "Сначала задайте TLS-домен — потом устанавливайте утилиты",
		"Utility": "Утилита",
		"Install": "Установить",
		"Uninstall": "Удалить",
		"Install all": "Установить всё",
		"Issue/renew certificate": "Выпустить/обновить сертификат",
		"Sync subscriptions": "Синхронизировать подписки",
		"Install the subscription statics utility first": "Сначала установите утилиту статики подписок",
		"Subscription files re-rendered and pushed": "Файлы подписок перерисованы и запушены",
		"Install caddy + acme and set a TLS domain first": "Сначала установите caddy + acme и задайте TLS-домен",
		"Uninstall this utility?": "Удалить эту утилиту?",
		"stale": "устарело",
		"stale hint": "Запушенные артефакты отстали от стора — примени снова",
		"SANs: ": "SANs: ",
		"SSH connect failed: ": "SSH-подключение не удалось: ",
		"Invalid TLS domain — use a bare hostname like node1.example.com": "Некорректный TLS-домен — укажите имя вида node1.example.com",
		"Caddy (SNI router, 80/443)": "Caddy (SNI-роутер, 80/443)",
		"ACME (acme.sh certificates)": "ACME (сертификаты acme.sh)",
		"Fake site (camouflage)": "Фейк-сайт (маскировка)",
		"Subscription statics": "Статика подписок",
		"Owns ports 80/443: routes TLS by SNI to the protocol inbounds, passes everything else to Reality, serves the site and subscriptions.": "Владеет портами 80/443: маршрутизирует TLS по SNI на инбаунды протоколов, остальное пропускает на Reality, раздаёт сайт и подписки.",
		...
		"Bot admin IDs": "Telegram ID админов (через запятую)",
		"Bot notify chat": "ID чата для уведомлений",
		"Bot restart note": "Бот перезапускается при сохранении настроек.",
		"Bot help": "Команды: /status — мой статус, /config — ссылки подписок. Админ: /nodes, /users, /online, /link <юзер>.",
		"Bot start hint": "Отправьте /start <код> для привязки. Код выдаёт оператор через /link. Ваш Telegram ID: %d",
		"Bot bound": "Привязан к пользователю %s. /status — статус, /config — подписки.",
		"Bot code not found": "Код неизвестен или истёк. Попросите у оператора новый (/link).",
		"Bot not bound": "Аккаунт не привязан к пользователю. Отправьте /start <код> с кодом от оператора.",
		"Bot config no nodes": "Ни одна нода пока не раздаёт подписки (установите утилиту sub). Ваш токен:",
		"Bot config links": "Ваши подписки:",
		"Bot link usage": "Формат: /link <имя или ID юзера>",
		"Bot user not found": "Пользователь %q не найден",
		"Bot link issued": "Код для %s: %s — пользователь должен отправить: /start %s",
		"Bot online none": "Онлайн никого (%d активных)",
		"Bot online list": "Онлайн %d/%d: %s",
		"Bot nodes none": "Ноды не зарегистрированы",
		"Bot users summary": "Юзеры: %d (активных %d, ждут активации %d, истекло %d)",
		"Bot expiry warning": "Юзер %s истекает %s (через ~%dч)",
		...
		"Bulk Add Users": "Массовое добавление юзеров",
		"Count": "Количество",
		"Name prefix": "Префикс имени",
		"Expires in (days)": "Истекает через (дней)",
		"0 = no expiry": "0 = без истечения",
		"Data limit (GB)": "Лимит трафика (ГБ)",
		"0 = unlimited": "0 = без лимита",
		"No chains yet": "Цепочек пока нет",
		"%d users created (prefix %q) — refresh to see them": "Создано юзеров: %d (префикс %q) — обновите страницу",
		"Count must be 1..100": "Количество должно быть 1..100",
		"no users were created (check chains/store)": "ни один юзер не создан (проверьте цепочки/стор)",
		"Panel relay: ON": "Реле панели: ВКЛ",
		...
		"Panel import reads the panel's SQLite DB and brings over its users, inbounds and routing rules, then stops the panel service. The raw DB is backed up first. Wipe stops the panel without importing.": "Импорт панели читает её SQLite-БД и переносит юзеров, инбаунды и правила маршрутизации, затем останавливает сервис панели. Сначала создаётся бэкап БД. Стирание останавливает панель без импорта.",
		"Import this panel's data? The panel service will be stopped.": "Импортировать данные панели? Сервис панели будет остановлен.",
		"Import panel": "Импортировать панель",
		"Stop the panel without importing?": "Остановить панель без импорта?",
		"Wipe panel": "Стереть панель",
		"Panel import done": "Импорт панели завершён",
		"Imported: %d inbound(s), %d user(s), %d routing rule(s).": "Импортировано: инбаундов %d, юзеров %d, правил маршрутизации %d.",
		"Panel DB download failed: ": "Не удалось скачать БД панели: ",
		"Panel DB parse failed: ": "Не удалось разобрать БД панели: ",
		"Panel DB backed up failed (continuing): ": "Бэкап БД панели не удался (продолжаем): ",
		"Panel import needs the built-in SSH client (download support).": "Импорт панели требует встроенный SSH-клиент (поддержка загрузки файлов).",
		"save node: ": "сохранение ноды: ",
		"Import details": "Детали импорта",
		"Next: review the imported inbounds and Apply the node to deploy sing-box.": "Дальше: проверьте импортированные инбаунды и сделайте Apply ноды для деплоя sing-box.",
		"Panel import result": "Результат импорта панели",
		"Panel wipe": "Стирание панели",
		"%d route(s)": "%d правил",
		"Open": "Открыть",
		"Subscription": "Подписка",
		"Sub page hint": "Вставьте vpn:// в AmneziaVPN или добавьте ?format=clash / ?format=base64 для клиентов.",
		"Amnezia vpn://": "Amnezia vpn://",
		"Share links": "Ссылки",
		"AWG outbound": "AWG outbound",
		"AWG outbound hint": "Вставьте клиентский .conf или vpn:// — нода станет клиентом чужого AWG (awgo-N, Table=off).",
		"Add AWG outbound": "Добавить AWG outbound",
		"invalid port": "некорректный порт",
		"no free port available": "нет свободных портов",
		"inbound profile not found": "инбаунд не найден",
		"Profile saved, but removal on %s was refused: a chain references it there": "Инбаунд сохранён, но снятие с %s отклонено: на него ссылается цепочка",
		"Protocol change requires recreating the profile (delete + create): deployed credentials are protocol-specific": "Смена протокола требует пересоздания инбаунда (удалить + создать): развёрнутые креды специфичны для протокола",
		"Profile is in use by a chain on %s — edit the chain first": "Инбаунд используется цепочкой на %s — сначала отредактируйте цепочку",
		"After connect": "После подключения",
		"Deploy sing-box-extended (binary)": "Развернуть sing-box-extended (бинарник)",
		"Install AmneziaWG kernel module": "Установить kernel-модуль AmneziaWG",
		"Detect existing VPN (takeover wizard)": "Найти существующий VPN (мастер takeover)",
		"binary/AWG install queued": "установка бинарника/AWG в очереди",
		"Detect VPN": "Найти VPN",
		// v0.8 — chain levels editor.
		"Transport (inter-node)": "Транспорт (между нодами)",
		"AWG / AmneziaWG (single-node levels only)": "AWG / AmneziaWG (только уровни из 1 ноды)",
		"Transport preset (inter-node obfuscation)": "Пресет транспорта (обфускация между нодами)",
		"Levels (entry first, exit last)": "Уровни (первый — вход, последний — выход)",
		"+ Add level": "+ Уровень",
		"Traffic flows level by level. A level with several nodes balances across them per its strategy. The entry level references inbounds already deployed on its nodes — create them on the Inbounds page first.": "Трафик идёт по уровням. Уровень с несколькими нодами балансирует между ними по своей стратегии. Уровень входа ссылается на инбаунды, уже развёрнутые на его нодах, — сначала создайте их на странице «Инбаунды».",
		"Level %d — Entry": "Уровень %d — вход",
		"Level %d": "Уровень %d",
		"Round-robin (fallback)": "Round-robin (fallback)",
		"— choose inbound —": "— выберите инбаунд —",
		"No inbound deployed on this node.": "На этой ноде нет развёрнутого инбаунда.",
		"Create / deploy one": "Создать / развернуть",
		"Chain name is required": "Укажите имя цепочки",
		"chain already exists": "цепочка уже существует",
		"level %d has no nodes selected": "на уровне %d не выбраны ноды",
		"level %d: invalid strategy %q": "уровень %d: некорректная стратегия %q",
		"entry node %q has no inbound selected — deploy one on the Inbounds page first": "для ноды входа %q не выбран инбаунд — сначала разверните его на странице «Инбаунды»",
		"entry node %q: inbound profile %q not found": "нода входа %q: инбаунд %q не найден",
		"inbound %q is not deployed on node %q — deploy it on the Inbounds page first": "инбаунд %q не развёрнут на ноде %q — разверните его на странице «Инбаунды»",
		"entry nodes must use inbounds of one protocol (%s vs %s)": "ноды входа должны использовать инбаунды одного протокола (%s vs %s)",
		"at least one level is required": "нужен хотя бы один уровень",
		"entry level is required": "нужен уровень входа",
		"Topology": "Топология",
		"levels": "уровни",
		...
		"%d not healthy": "%d нездоровы",
		"all healthy": "все здоровы",
