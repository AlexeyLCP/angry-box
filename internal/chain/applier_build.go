...
func (a *Applier) ApplyChain(ctx context.Context, store *Store, chain *model.Chain, awgClientPubKey string) (*ApplyReport, error) {
	...
	if chain.UserProtocol == model.UserProtocolTUIC {
		if chain.TUICEntryUserUUID == "" || chain.TUICEntryUserPassword == "" {
			...
			if err != nil {
				return nil, fmt.Errorf("apply chain: generate TUIC entry creds: %w", err)
			}
...
// used by route rules.
func chainTUICUsers(c *model.Chain, users []model.User) ([]config.TUICUser, error) {
	...
	if len(out) == 0 {
		...
		return []config.TUICUser{{UUID: tuicUUID(c), Password: pw}}, nil
	}
	return out, nil
}
...
// Returns an error if the password generator fails instead of panicking in the
// request path (CTO-review #3).
func GenerateStableTUICUserCreds() (uuid, password string, err error) {
	uuid = generateStableUUID()
	password, err = GenerateTUICPassword()
...
func (a *Applier) applyMergedNodeLocked(
	...
	if needsAWGModule {
		...
		if fwdErr := ensureIPForward(ctx, client, info.UseSudo); fwdErr != nil {
